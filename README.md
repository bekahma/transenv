...
# Row chunking transforms each text as one unit and avoids the expensive sentence/hybrid fan-out.
...
python src/run/main.py --batch_size 5 --openai_parallelism 2 --max_samples 20 --text_chunking row --max_rules_per_chunk 2 --max_rules_per_row 2 --save_path ./outputs/cefr_texts/dialect --file_name aave_gpt41mini_row_smoke20 --input_path ./data/cefr_leveled_texts.csv --text_column text --input_cefr_levels A1,A2 --dialect "Urban African American Vernacular English" --task_name english_dialect --data_path ./ --dataset_name cefr_texts --model_provider openai --model_name gpt-4.1-mini

# Post-filter completed CEFR text transformations for CAA/vector extraction
python src/run/filter_cefr_outputs.py --input_csv ./outputs/cefr_texts/dialect/aave_gpt41mini_row_smoke20.csv --output_dir ./outputs/cefr_texts/dialect/filtered --file_prefix aave_gpt41mini_row_smoke20 --max_edit_rate 0.10
```
