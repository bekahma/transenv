...
OPENAI_CALL_MODE=${OPENAI_CALL_MODE:-sync}
OPENAI_BATCH_POLL_INTERVAL=${OPENAI_BATCH_POLL_INTERVAL:-60}
TEXT_CHUNKING=${TEXT_CHUNKING:-row}
SENTENCE_CHUNK_MIN_WORDS=${SENTENCE_CHUNK_MIN_WORDS:-100}
MAX_CHUNK_WORDS=${MAX_CHUNK_WORDS:-80}
CAA_MAX_EDIT_RATE=${CAA_MAX_EDIT_RATE:-0.25}
CAA_MIN_LENGTH_RATIO=${CAA_MIN_LENGTH_RATIO:-0.50}
CAA_MAX_LENGTH_RATIO=${CAA_MAX_LENGTH_RATIO:-1.80}
...
echo "OPENAI_CALL_MODE: $OPENAI_CALL_MODE"
echo "OPENAI_BATCH_POLL_INTERVAL: $OPENAI_BATCH_POLL_INTERVAL"
...
echo "CAA_MAX_EDIT_RATE: $CAA_MAX_EDIT_RATE"
echo "CAA_MIN_LENGTH_RATIO: $CAA_MIN_LENGTH_RATIO"
echo "CAA_MAX_LENGTH_RATIO: $CAA_MAX_LENGTH_RATIO"
...
python src/run/main.py \
  ...
  "${WRITE_CAA_PAIRS_ARG[@]}" \
  --caa_max_edit_rate "$CAA_MAX_EDIT_RATE" \
  --caa_min_length_ratio "$CAA_MIN_LENGTH_RATIO" \
  --caa_max_length_ratio "$CAA_MAX_LENGTH_RATIO" \
  --openai_parallelism "$OPENAI_PARALLELISM" \
  --openai_call_mode "$OPENAI_CALL_MODE" \
